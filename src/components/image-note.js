...
class ImageNote extends Component {
  ...
  save = () => {
    var self = this
    this.getImage(this.refs.src.value).then(function(img) {
      let imgHeight = 150
      let imgWidth = img.width / img.height * 150
      self.props.onImgNoteSave(
        self.refs.src.value,
        self.props.id,
        imgWidth,
        imgHeight
  ...
  displayMode = () => {
    let { src, imgHeight, imgWidth } = this.props
    return (
  ...
  render() {
    return (
      <Draggable
        ...
        onDrag={(e, position) => this.drag(e, position)}
      >
